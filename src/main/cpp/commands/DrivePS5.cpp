...
DrivePS5::DrivePS5(frc2::CommandJoystick *joystick, SwerveDrive *swerveDrive, DriveState driveState, units::degree_t arbitraryAngle) :
m_bill(joystick),
...
units::angular_velocity::radians_per_second_t DrivePS5::GetDesiredRotationalVelocity() {
...
void DrivePS5::Initialize() {}

// Called repeatedly when this Command is scheduled to run
void DrivePS5::Execute() {
  UpdatePreferences();
  ...
  // the controller values get swapped (-m_bill) to DrivePS5 field relative based on alliance
  ...
  // based on DrivePS5 state, set the rotation and velocity to do so

  m_swerveDrive->Drive(xSpeed, ySpeed, rot, true, frc::Translation2d{});
}

// Called once the command ends or is interrupted.
void DrivePS5::End(bool interrupted) {
  m_swerveDrive->Stop();
}

// Returns true when the command should end.
bool DrivePS5::IsFinished() {
  return false;
}

units::angular_velocity::radians_per_second_t DrivePS5::GetRotVelSpeakerAlign() {
  m_allianceSide = frc::DriverStation::GetAlliance();
...
//for pid tunings
void DrivePS5::UpdatePreferences() {
